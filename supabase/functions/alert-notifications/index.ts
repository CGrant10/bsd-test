...
Deno.serve(async request => {
  ...
  try {
    ...
    if (alert.status !== "active" || !alert.is_public) {
    ...
    if (subscriptionsError) throw subscriptionsError;
    const title = alert.child_name ? `Community alert: ${alert.child_name}` : (alert.title || "BSD #7 Community Alert");
    const body = String(alert.description || "A new verified community alert is available.").trim().slice(0, 180);
